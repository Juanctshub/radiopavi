...
function setupEvents() {
    ...
    if (DOM.btnAdminAddTracks) {
        DOM.btnAdminAddTracks.addEventListener('click', () => { 
            // Force interface to jump to the local playlist tab and ask for files
            DOM.navTabs.forEach(t=>t.classList.remove('active')); DOM.navTabs[0].classList.add('active');
            DOM.viewLocal.style.display='flex'; DOM.viewCloud.style.display='none'; DOM.viewBroadcast.style.display='none'; DOM.viewIntercom.style.display='none';
            DOM.fileInput.click(); 
        });
    }
