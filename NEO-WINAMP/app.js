...
// ROLES & P2P
let appRole = null; // 'admin' or 'listener'
let peer = null;
let p2pConnections = []; // stores { conn, call }
let lastTrackName = 'RADIO-PAVI >> AWAITING SIGNAL...';
...
function queryDOM() {
    DOM = {
        roleModal:document.getElementById('role-modal'), tuneInModal:document.getElementById('tune-in-modal'),
        btnRoleListen:document.getElementById('btn-role-listener'), btnRoleAdmin:document.getElementById('btn-role-admin'),
        adminLoginDiv:document.getElementById('admin-login'), adminPwd:document.getElementById('admin-pwd'),
        btnSubmitLogin:document.getElementById('btn-login-submit'), loginErr:document.getElementById('login-error'),
        btnTuneIn:document.getElementById('btn-tune-in'), tuneStatus:document.getElementById('tune-status'),
        listenerCount:document.getElementById('listener-count'), listenerVol:document.getElementById('listener-vol'),
        ...
        btnMin:document.getElementById('btn-min'), btnClose:document.getElementById('btn-close'),
        btnCfg:document.getElementById('btn-cfg'),
        volSlider:document.getElementById('volume-slider'), balSlider:document.getElementById('balance-slider'),
        progSlider:document.getElementById('progress-slider'),
        timeDisplay:document.getElementById('time-display'), trackName:document.getElementById('track-name'),
        playlistEl:document.getElementById('playlist'), dropZone:document.getElementById('drop-zone'),
        statsEl:document.getElementById('playlist-stats'), ambientGlow:document.getElementById('ambient-glow'),
        btnAdd:document.getElementById('btn-add'), btnAddFolder:document.getElementById('btn-add-folder'),
        btnRem:document.getElementById('btn-rem'), btnClear:document.getElementById('btn-clear'),
        navTabs:document.querySelectorAll('.tab'), viewLocal:document.getElementById('view-local'), viewCloud:document.getElementById('view-cloud'),
        apiInput:document.getElementById('api-search-input'), btnApiSearch:document.getElementById('btn-api-search'), btnAddUrl:document.getElementById('btn-add-url'),
        apiResults:document.getElementById('api-results'), urlStatus:document.getElementById('url-status'),
        ...
        canvas:document.getElementById('visualizer'), nCanvas:document.getElementById('neural-overlay'),
        scanlineOverlay:document.getElementById('scanline-overlay'), bgEffect:document.getElementById('bg-effect'),
        cfgScanlines:document.getElementById('cfg-scanlines'), cfgGlow:document.getElementById('cfg-glow'), swatchRow:document.getElementById('swatch-row'),
        vizModes:document.getElementById('viz-modes'), bgModes:document.getElementById('bg-modes'),
        // ON AIR
        btnOnAir:document.getElementById('btn-on-air'), airLabel:document.getElementById('air-label'),
        airDot:document.getElementById('air-dot'), micLevel:document.getElementById('mic-level'),
        duckSlider:document.getElementById('duck-slider'), duckVal:document.getElementById('duck-val')
    };
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.body.className = ''; // Reset roles
    
    // Auth Flow
    DOM.btnRoleListen.addEventListener('click', () => initApp('listener'));
    DOM.btnRoleAdmin.addEventListener('click', () => DOM.adminLoginDiv.style.display='block');
    DOM.btnSubmitLogin.addEventListener('click', () => {
        if(DOM.adminPwd.value === 'admin123') initApp('admin');
        else DOM.loginErr.textContent = 'ACCESS DENIED: Invalid Passkey';
    });
});
...
async function initApp(role) {
    ...
    if (role === 'admin') {
        ...
        // Listener mode bypasses splash, goes to Tune In screen
        DOM.tuneInModal.style.display = 'flex';
        initListenerPeer();
    }
}

// ═══════════════════════════ PEER.JS NETWORK LOGIC ═══════════════════════════
...
function updateAdminListenerCount() {
    ...
    DOM.listenerCount.textContent = active + ' LSTN';
...
function initAdminPeer() {
    peer = new Peer('radiopavi-admin'); // Fixed host ID
    peer.on('open', id => console.log('Admin Peer Online:', id));
    
    peer.on('connection', conn => {
        // New Listener Data Connection
        ...
        conn.on('open', () => {
            ...
            // Call them back with our audio stream immediately
...
function initListenerPeer() {
    ...
    peer.on('open', id => {
        DOM.tuneStatus.textContent = 'SIGNAL FOUND (radiopavi-admin)';
        DOM.tuneStatus.classList.remove('blink');
        DOM.btnTuneIn.style.display = 'inline-block';
    });

    peer.on('error', err => {
        DOM.tuneStatus.textContent = 'ERROR: SIGNAL LOST';
        DOM.tuneStatus.style.color = 'var(--red)';
        console.error(err);
    });

    // Listen for the incoming audio call from Admin
    peer.on('call', call => {
    ...
    DOM.btnTuneIn.addEventListener('click', () => {
        // Require user interaction to start Web Audio
        DOM.tuneStatus.textContent = 'CONNECTING...';
        const conn = peer.connect('radiopavi-admin');
        
        conn.on('open', () => {
             // Admin will receive connection and call us back
             console.log('Data link to admin open');
             DOM.tuneInModal.style.display = 'none';
        ...
        conn.on('data', data => {
            if(data.type === 'metadata') {
                DOM.trackName.textContent = data.track.toUpperCase();
            } else if(data.type === 'stats') {
                DOM.listenerCount.textContent = data.listeners + ' LSTN';
                DOM.listenerCount.style.display = 'inline-block';
            }
        });
    });
}
...
function startListenerWebAudio(stream) {
    ...
    // We must resume audio context on a user gesture, which btnTuneIn provided.
    audioCtx.resume(); 
    ...
    analyser.connect(volNode);
    volNode.connect(audioCtx.destination);

    // Tie Listener Vol slider
    DOM.listenerVol.addEventListener('input', e => volNode.gain.value = e.target.value);

    // Hijack the visualizer engine
...
function initAudio() {
    ...
    // EQ -> musicGain -> Panner -> Analyser -> Speakers
    ...
    // Audio also routes to Broadcast Stream Destination (Post EQ & Ducking, Pre-Panner)
...
function stopAudio(){audioParams.audioElement.pause();audioParams.audioElement.currentTime=0;audioParams.isPlaying=false;updateTransportUI('stop');drawIdleScreen();setBroadcastTrackName('RADIO-PAVI >> OFF AIR');}
...
async function goOnAir() {
    ...
    DOM.btnOnAir.classList.add('active'); DOM.airLabel.textContent = 'LIVE';
...
function goOffAir() {
    ...
    DOM.btnOnAir.classList.remove('active'); DOM.airLabel.textContent = 'ON AIR';
...
function startMicMeter() {
    ...
    function update() {
        ...
        DOM.micLevel.style.width = Math.min(100, ((sum / data.length) / 100) * 100) + '%';
    }
    update();
}
function stopMicMeter() { if (micMeterFrame) cancelAnimationFrame(micMeterFrame); DOM.micLevel.style.width = '0%'; }

// ═══════════════════════════ EVENTS (ADMIN ONLY) ═══════════════════════════
function setupAdminEvents() {
    // Transport
    DOM.btnPlay.addEventListener('click',()=>{flash(DOM.btnPlay);if(playlist.length===0)DOM.fileInput.click();else if(!audioParams.isPlaying){if(currentIndex===-1)loadTrack(0);else playAudio();}});
    DOM.btnPause.addEventListener('click',()=>{flash(DOM.btnPause);if(audioParams.isPlaying)pauseAudio();else if(currentIndex>=0)playAudio();});
    DOM.btnStop.addEventListener('click',()=>{flash(DOM.btnStop);stopAudio();});
    DOM.btnNext.addEventListener('click',()=>{flash(DOM.btnNext);if(!playlist.length)return;if(isShuffle)loadTrack(Math.floor(Math.random()*playlist.length));else if(currentIndex<playlist.length-1)loadTrack(currentIndex+1);else loadTrack(0);});
    DOM.btnPrev.addEventListener('click',()=>{flash(DOM.btnPrev);if(!playlist.length)return;if(audioParams.audioElement.currentTime>3)audioParams.audioElement.currentTime=0;else if(currentIndex>0)loadTrack(currentIndex-1);else audioParams.audioElement.currentTime=0;});
    ...
    DOM.btnCfg.addEventListener('click',()=>DOM.cfgDeck.classList.toggle('cfg-open'));

    // ON AIR
    DOM.btnOnAir.addEventListener('click', () => { if(isOnAir) goOffAir(); else goOnAir(); });
    DOM.duckSlider.addEventListener('input', (e) => {
    ...
    // Time + ended (Only Admin drives playback)
    ...
    DOM.navTabs.forEach(tab=>{tab.addEventListener('click',()=>{DOM.navTabs.forEach(t=>t.classList.remove('active'));tab.classList.add('active');DOM.viewLocal.style.display=tab.dataset.target==='local'?'flex':'none';DOM.viewCloud.style.display=tab.dataset.target==='cloud'?'flex':'none';});});
...
function testAndAddURL(){
    const url=DOM.apiInput.value.trim(); if(!url||(!url.startsWith('http://')&&!url.startsWith('https://'))){setStatus('⚠ Enter a valid URL','error');return;}
...
function startVisualizer(){
    ...
    function draw(){
        if(!audioParams.isPlaying && !isOnAir && appRole !== 'listener') return;
...
function startMicVisualizer() { startVisualizer(); } // Handled by unified loop now
...
function resetChameleon(){DOM.ambientGlow.style.background='';}
